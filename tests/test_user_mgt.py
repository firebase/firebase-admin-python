...
EMULATED_ID_TOOLKIT_URL = "http://{}/identitytoolkit.googleapis.com/v1".format(
    AUTH_EMULATOR_HOST
)
...
def user_mgt_app(request):
    ...
    if request.param["emulated"]:
        ...
        monkeypatch.setitem(
            USER_MGT_URLS, "PREFIX", EMULATED_ID_TOOLKIT_URL + URL_PROJECT_SUFFIX
        )
...
def _instrument_user_manager(app, status, payload):
    ...
    user_manager.http_client.session.mount(
        USER_MGT_URLS["ID_TOOLKIT"], testutils.MockAdapter(payload, status, recorder)
    )
...
class TestGetUser:
    ...
    def test_get_user(self, user_mgt_app):
        _, recorder = _instrument_user_manager(
            user_mgt_app, 200, MOCK_GET_USER_RESPONSE
        )
    ...
    def test_get_user_with_timeout(self, user_mgt_app_with_timeout):
        ...
        _check_request(
            recorder, "/accounts:lookup", {"localId": ["testuser"]}, TEST_TIMEOUT
        )
    ...
    def test_get_user_by_email(self, user_mgt_app):
        ...
        _check_request(
            recorder, "/accounts:lookup", {"email": ["testuser@example.com"]}
        )
    ...
    def test_get_user_http_error_unexpected_code(self, user_mgt_app):
        ...
        with pytest.raises(exceptions.InternalError) as excinfo:
            auth.get_user("testuser", user_mgt_app)
        assert (
            str(excinfo.value) == "Error while calling Auth service (UNEXPECTED_CODE)."
        )
        assert excinfo.value.http_response is not None
        assert excinfo.value.cause is not None
    ...
    def test_get_user_http_error_malformed_response(self, user_mgt_app):
        ...
        assert (
            str(excinfo.value)
            == 'Unexpected error response: {"error": "UNEXPECTED_CODE"}'
        )
        assert excinfo.value.http_response is not None
        assert excinfo.value.cause is not None
    ...
    def test_success(self, user_mgt_app):
        mock_users = [
            ...
            },
            {
                ...
                "providerUserInfo": [
                    {"providerId": "google.com", "rawId": "google_uid4"}
                ],
            },
        ]
        _instrument_user_manager(
            user_mgt_app, 200, '{ "users": ' + json.dumps(mock_users) + "}"
        )
        ...
        get_users_results = auth.get_users(
            [
                ...
                auth.ProviderIdentifier(
                    provider_id="google.com", provider_uid="google_uid4"
                ),
        ...
        assert [u.uid for u in get_users_results.not_found] == [
...
class TestCreateUser:
    ...
    def test_create_user(self, user_mgt_app):
        user_mgt, recorder = _instrument_user_manager(
            user_mgt_app, 200, '{"localId":"testuser"}'
        )
    ...
    def test_create_user_with_email(self, user_mgt_app):
        ...
        assert (
            user_mgt.create_user(email="test@example.com", email_verified=True)
            == "testuser"
        )
        request = json.loads(recorder[0].body.decode())
    ...
    def test_user_already_exists(self, user_mgt_app, error_code):
        ...
        assert str(excinfo.value) == "{0} ({1}).".format(
            exc_type.default_message, error_code
        )
        assert excinfo.value.http_response is not None
        assert excinfo.value.cause is not None
...
class TestUpdateUser:
    ...
    @pytest.mark.parametrize(
        "arg", [[], ["phone"], ["google.com"], ["google.com", "phone"]]
    )
    def test_update_user_delete_provider_and_phone(self, user_mgt_app, arg):
        ...
        user_mgt.update_user(
            "testuser", providers_to_delete=arg, phone_number=auth.DELETE_ATTRIBUTE
        )
        request = json.loads(recorder[0].body.decode())
...
class TestSetCustomUserClaims:
    ...
    def test_single_reserved_claim(self, user_mgt_app, key):
        ...
        assert str(
            excinfo.value
        ) == 'Claim "{0}" is reserved, and must not be set.'.format(key)
    ...
    def test_large_claims_payload(self, user_mgt_app):
        ...
        assert (
            str(excinfo.value)
            == "Custom claims payload must not exceed 1000 characters."
        )
...
class TestDeleteUsers:
    ...
    def test_should_index_errors_correctly_in_results(self, user_mgt_app):
        ...
        delete_users_result = auth.delete_users(
            ["uid1", "uid2", "uid3", "uid4"], app=user_mgt_app
        )
    ...
    def test_success(self, user_mgt_app):
        _instrument_user_manager(user_mgt_app, 200, "{}")
        delete_users_result = auth.delete_users(
            ["uid1", "uid2", "uid3"], app=user_mgt_app
        )
...
class TestListUsers:
    ...
    def test_list_single_page(self, user_mgt_app):
        _, recorder = _instrument_user_manager(
            user_mgt_app, 200, MOCK_LIST_USERS_RESPONSE
        )
    ...
    def test_list_multiple_pages(self, user_mgt_app):
        ...
        self._check_rpc_calls(
            recorder, {"maxResults": "1000", "nextPageToken": "token"}
        )
    ...
    def test_permission_error(self, user_mgt_app):
        ...
        msg = (
            "The credential used to initialize the SDK has insufficient "
...
class TestUserMetadata:
    _INVALID_ARGS = [{"creation_timestamp": arg} for arg in INVALID_TIMESTAMPS] + [
...
class TestImportUserRecord:
    _INVALID_USERS = (
        ...
        + [
            {"custom_claims": arg}
            for arg in INVALID_DICTS[1:] + ['"json"', {"key": "a" * 1000}]
        ]
...
class TestUserImportHash:
    ...
    @pytest.mark.parametrize(
        "func",
        [
            auth.UserImportHash.hmac_sha512,
            auth.UserImportHash.hmac_sha256,
            auth.UserImportHash.hmac_sha1,
            auth.UserImportHash.hmac_md5,
        ],
    )
...
class TestImportUsers:
    ...
    def test_import_users_http_error(self, user_mgt_app):
        _instrument_user_manager(
            user_mgt_app, 401, '{"error": {"message": "ERROR_CODE"}}'
        )
        users = [
            auth.ImportUserRecord(uid="user1"),
            auth.ImportUserRecord(uid="user2"),
...
class TestGenerateEmailActionLink:
    def test_email_verification_no_settings(self, user_mgt_app):
        _, recorder = _instrument_user_manager(
            user_mgt_app, 200, '{"oobLink":"https://testlink"}'
        )
    ...
    def test_password_reset_non_existing(self, user_mgt_app):
        _instrument_user_manager(
            user_mgt_app, 400, '{"error":{"message": "EMAIL_NOT_FOUND"}}'
        )
    ...
    def test_invalid_dynamic_link(self, user_mgt_app, func):
        ...
        assert str(excinfo.value) == (
            "Dynamic link domain specified in ActionCodeSettings is "
