...
class _ListOIDCProviderConfigsPage(ListProviderConfigsPage):
    ...
    def provider_configs(self):
        return [
            ...
            for data in self._current.get("oauthIdpConfigs", [])
        ]
...
class _ListSAMLProviderConfigsPage(ListProviderConfigsPage):
    ...
    def provider_configs(self):
        return [
            ...
            for data in self._current.get("inboundSamlConfigs", [])
        ]
...
class ProviderConfigClient:
    ...
    def list_oidc_provider_configs(
        self, page_token=None, max_results=MAX_LIST_CONFIGS_RESULTS
    ):
    ...
    def create_saml_provider_config(
        ...
        req = {
            "idpConfig": {
                "idpEntityId": _validate_non_empty_string(
    ...
    def _fetch_saml_provider_configs(
        ...
        return self._fetch_provider_configs(
            "/inboundSamlConfigs", page_token, max_results
        )

    def _fetch_provider_configs(
        self, path, page_token=None, max_results=MAX_LIST_CONFIGS_RESULTS
    ):
...
def _validate_url(url, label):
    ...
    if not isinstance(url, str) or not url:
        raise ValueError(
            'Invalid photo URL: "{0}". {1} must be a non-empty ' "string.".format(
                url, label
            )
        )
    try:
