...
class MulticastMessage:
    ...
    def __init__(
        ...
        if len(tokens) > 500:
            raise ValueError(
                "MulticastMessage.tokens must not contain more than 500 tokens."
            )
        self.tokens = tokens
        self.data = data
        self.notification = notification
...
class MessageEncoder(json.JSONEncoder):
    ...
    def encode_android(cls, android):
        ...
        result = {
            ...
            "data": _Validators.check_string_dict("AndroidConfig.data", android.data),
    ...
    def encode_android_fcm_options(cls, fcm_options):
        ...
        if not isinstance(fcm_options, _messaging_utils.AndroidFCMOptions):
            raise ValueError(
                "AndroidConfig.fcm_options must be an instance of "
    ...
    def encode_ttl(cls, ttl):
        ...
        if not isinstance(ttl, datetime.timedelta):
            raise ValueError(
                "AndroidConfig.ttl must be a duration in seconds or an instance of "
                "datetime.timedelta."
            )
    ...
    def encode_milliseconds(cls, label, msec):
        ...
        if not isinstance(msec, datetime.timedelta):
            raise ValueError(
                "{0} must be a duration in milliseconds or an instance of "
    ...
    def encode_android_notification(cls, notification):
        ...
        if not isinstance(notification, _messaging_utils.AndroidNotification):
            raise ValueError(
                "AndroidConfig.notification must be an instance of "
        ...
        result = {
            ...
            "image": _Validators.check_string("image", notification.image),
            ...
            "vibrate_timings": _Validators.check_number_list(
                "AndroidNotification.vibrate_timings_millis",
            ...
            "notification_count": _Validators.check_number(
                "AndroidNotification.notification_count",
        ...
        if priority:
            if priority not in ("min", "low", "default", "high", "max"):
                raise ValueError(
                    'AndroidNotification.priority must be "default", "min", "low", '
                    '"high" or "max".'
                )
    ...
    def encode_light_settings(cls, light_settings):
        ...
        result = {
            ...
            "light_on_duration": cls.encode_milliseconds(
                "LightSettings.light_on_duration_millis",
                light_settings.light_on_duration_millis,
            ),
        ...
        if not light_on_duration:
            raise ValueError("LightSettings.light_on_duration_millis is required.")

        light_off_duration = result.get("light_off_duration")
        if not light_off_duration:
            raise ValueError("LightSettings.light_off_duration_millis is required.")

        color = result.get("color")
        if not color:
            raise ValueError("LightSettings.color is required.")
        if not re.match(r"^#[0-9a-fA-F]{6}$", color) and not re.match(
            r"^#[0-9a-fA-F]{8}$", color
        ):
            raise ValueError(
    ...
    def encode_webpush(cls, webpush):
        ...
        result = {
            "data": _Validators.check_string_dict("WebpushConfig.data", webpush.data),
    ...
    def encode_webpush_notification(cls, notification):
        ...
        if not isinstance(notification, _messaging_utils.WebpushNotification):
            raise ValueError(
                "WebpushConfig.notification must be an instance of "
        ...
        if notification.custom_data is not None:
            ...
            for key, value in notification.custom_data.items():
                if key in result:
                    raise ValueError(
                        "Multiple specifications for {0} in WebpushNotification.".format(
                            key
                        )
                    )
                result[key] = value
    ...
    def encode_webpush_fcm_options(cls, options):
        ...
        result = {
            "link": _Validators.check_string(
                "WebpushConfig.fcm_options.link", options.link
            ),
        }
        result = cls.remove_null_values(result)
    ...
    def encode_apns_payload(cls, payload):
        ...
        if not isinstance(payload, _messaging_utils.APNSPayload):
            raise ValueError(
                "APNSConfig.payload must be an instance of APNSPayload class."
            )
    ...
    def encode_apns_fcm_options(cls, fcm_options):
        ...
        if not isinstance(fcm_options, _messaging_utils.APNSFCMOptions):
            raise ValueError(
                "APNSConfig.fcm_options must be an instance of APNSFCMOptions class."
            )
        result = {
            "analytics_label": _Validators.check_analytics_label(
            ...
            "image": _Validators.check_string(
                "APNSFCMOptions.image", fcm_options.image
            ),
        }
        result = cls.remove_null_values(result)
        return result
    ...
    def encode_aps_sound(cls, sound):
        ...
        result = {
            "name": _Validators.check_string(
                "CriticalSound.name", sound.name, non_empty=True
            ),
    ...
    def encode_aps_alert(cls, alert):
        ...
        if not isinstance(alert, _messaging_utils.ApsAlert):
            raise ValueError(
                "Aps.alert must be a string or an instance of ApsAlert class."
            )
        result = {
            ...
            "loc-key": _Validators.check_string("ApsAlert.loc_key", alert.loc_key),
        ...
        if result.get("loc-args") and not result.get("loc-key"):
            raise ValueError("ApsAlert.loc_key is required when specifying loc_args.")
    ...
    def encode_fcm_options(cls, fcm_options):
        ...
        if not isinstance(fcm_options, _messaging_utils.FCMOptions):
            raise ValueError(
                "Message.fcm_options must be an instance of FCMOptions class."
            )
        result = {
            "analytics_label": _Validators.check_analytics_label(
