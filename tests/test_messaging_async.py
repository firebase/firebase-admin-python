...
class TestSendAsync:
    ...
    async def test_no_project_id(self):
        async def evaluate():
            app = firebase_admin.initialize_app(testutils.MockCredentialAsync(), name='no_project_id')
...
class TestTopicManagementAsync:
    ...
    def _get_url(self, path):
        return '{0}/{1}'.format(messaging_async._MessagingServiceAsync.IID_URL, path)
