...
MODEL_JSON_1 = {"name": MODEL_NAME_1, "displayName": DISPLAY_NAME_1}
...
MODEL_JSON_2 = {"name": MODEL_NAME_2, "displayName": DISPLAY_NAME_2}
...
MODEL_JSON_3 = {"name": MODEL_NAME_3, "displayName": DISPLAY_NAME_3}
MODEL_3 = ml.Model.from_dict(MODEL_JSON_3)

MODEL_STATE_PUBLISHED_JSON = {"published": True}
...
TFLITE_FORMAT_JSON = {"gcsTfliteUri": GCS_TFLITE_URI, "sizeBytes": "1234567"}
...
GCS_TFLITE_SIGNED_URI = GCS_TFLITE_SIGNED_URI_PATTERN.format(
...
TFLITE_FORMAT_JSON_2 = {"gcsTfliteUri": GCS_TFLITE_URI_2, "sizeBytes": "2345678"}
TFLITE_FORMAT_2 = ml.TFLiteFormat.from_dict(TFLITE_FORMAT_JSON_2)

AUTOML_MODEL_NAME = (
    "projects/111111111111/locations/us-central1/models/ICN7683346839371803263"
)
AUTOML_MODEL_SOURCE = ml.TFLiteAutoMlSource(AUTOML_MODEL_NAME)
TFLITE_FORMAT_JSON_3 = {"automlModel": AUTOML_MODEL_NAME, "sizeBytes": "3456789"}
TFLITE_FORMAT_3 = ml.TFLiteFormat.from_dict(TFLITE_FORMAT_JSON_3)

AUTOML_MODEL_NAME_2 = (
    "projects/2222222222/locations/us-central1/models/ICN2222222222222222222"
)
...
OPERATION_DONE_MODEL_JSON_1 = {"done": True, "response": CREATED_UPDATED_MODEL_JSON_1}
...
DEFAULT_LIST_RESPONSE = json.dumps(
    {"models": [MODEL_JSON_1, MODEL_JSON_2], "nextPageToken": NEXT_PAGE_TOKEN}
)
LAST_PAGE_LIST_RESPONSE = json.dumps({"models": [MODEL_JSON_3]})
...
INVALID_MODEL_ARGS = ["abc", 4.2, list(), dict(), True, -1, 0, None]
...
PAGE_SIZE_VALUE_ERROR_MSG = (
    "Page size must be a positive integer between " "1 and {0}".format(
        ml._MAX_PAGE_SIZE
    )
)
...
class _TestStorageClient:
    ...
    def sign_uri(gcs_tflite_uri, app):
        del app  # unused variable
        bucket_name, blob_name = ml._CloudStorageClient._parse_gcs_tflite_uri(
            gcs_tflite_uri
        )
...
class TestModel:
    ...
    def test_model_keyword_based_creation_and_setters(self):
        model = ml.Model(
            display_name=DISPLAY_NAME_1, tags=TAGS, model_format=TFLITE_FORMAT
        )
        assert model.display_name == DISPLAY_NAME_1
        assert model.tags == TAGS
        assert model.model_format == TFLITE_FORMAT
    ...
    def test_gcs_tflite_model_format_source_creation(self):
        ...
        assert model.as_dict() == {
            "displayName": DISPLAY_NAME_1,
            "tfliteModel": {"gcsTfliteUri": GCS_TFLITE_URI},
        }
    ...
    def test_auto_ml_tflite_model_format_source_creation(self):
        ...
        assert model.as_dict() == {
            "displayName": DISPLAY_NAME_1,
            "tfliteModel": {"automlModel": AUTOML_MODEL_NAME},
        }
    ...
    def test_model_format_setters(self):
        ...
        assert model_format.as_dict() == {
            "tfliteModel": {"gcsTfliteUri": GCS_TFLITE_URI_2}
        }
    ...
    def test_model_as_dict_for_upload(self):
        ...
        assert model.as_dict(for_upload=True) == {
            "displayName": DISPLAY_NAME_1,
            "tfliteModel": {"gcsTfliteUri": GCS_TFLITE_SIGNED_URI},
        }
    ...
    @pytest.mark.parametrize(
        ...
        [("", ValueError), ("&_*#@:/?", ValueError), (12345, TypeError)],
    )
    ...
    @pytest.mark.parametrize("model_format", [123, "abc", {}, [], True])
    ...
    @pytest.mark.parametrize("model_source", [123, "abc", {}, [], True])
    def test_model_source_validation_errors(self, model_source):
        with pytest.raises(TypeError) as excinfo:
            ml.TFLiteFormat(model_source=model_source)
        check_error(
            excinfo, TypeError, "Model source must be a TFLiteModelSource object."
        )
    ...
    @pytest.mark.parametrize(
        "auto_ml_model, exc_type",
        [
            (123, TypeError),
            ("abc", ValueError),
            (
                "/projects/123456/locations/us-central1/models/noLeadingSlash",
                ValueError,
            ),
    ...
    def test_wait_for_unlocked(self):
        recorder = instrument_ml_service(
            status=200, operations=True, payload=OPERATION_DONE_PUBLISHED_RESPONSE
        )
    ...
    def test_wait_for_unlocked_timeout(self):
        ...
        ml._MLService.POLL_BASE_WAIT_TIME_SECONDS = (
            3  # longer so timeout applies immediately
        )
        model = ml.Model.from_dict(LOCKED_MODEL_JSON_1)
        with pytest.raises(Exception) as excinfo:
            model.wait_for_unlocked(max_time_seconds=0.1)
        check_error(
            excinfo, exceptions.DeadlineExceededError, "Polling max time exceeded."
        )
        assert len(recorder) == 1
...
class TestCreateModel:
    ...
    def test_operation_error(self):
        ...
        check_operation_error(
            excinfo, OPERATION_ERROR_EXPECTED_STATUS, OPERATION_ERROR_MSG
        )
    ...
    def test_malformed_operation(self):
        ...
        with pytest.raises(Exception) as excinfo:
            ml.create_model(MODEL_1)
        check_error(
            excinfo, exceptions.UnknownError, "Internal Error: Malformed Operation."
        )
...
class TestPublishUnpublish:
    ...
    def test_immediate_done(self, publish_function, published):
        ...
        assert len(recorder) == 1
        assert recorder[0].method == "PATCH"
        assert recorder[0].url == TestPublishUnpublish._update_url(
...
class TestGetModel:
    ...
    def test_get_model_error(self):
        ...
        check_firebase_error(
            excinfo, ERROR_STATUS_NOT_FOUND, ERROR_CODE_NOT_FOUND, ERROR_MSG_NOT_FOUND
        )
        assert len(recorder) == 1
        assert recorder[0].method == "GET"
        assert recorder[0].url == TestGetModel._url(PROJECT_ID, MODEL_ID_1)
...
class TestListModels:
    ...
    def test_list_models_with_all_args(self):
        ...
        models_page = ml.list_models(
            "display_name=displayName3", page_size=10, page_token=PAGE_TOKEN
        )
        assert len(recorder) == 1
        assert recorder[0].method == "GET"
        assert recorder[0].url == (
            TestListModels._url(PROJECT_ID)
            + "?filter=display_name%3DdisplayName3&page_size=10&page_token={0}".format(
                PAGE_TOKEN
            )
        )
        assert recorder[0].headers[HEADER_CLIENT_KEY] == HEADER_CLIENT_VALUE
    ...
    @pytest.mark.parametrize(
        "page_size, exc_type, error_message",
        [
