...
class TestAdvancedQueries:
    ...
    def test_limit_first(self, testref):
        value = (
            testref.child("dinosaurs").order_by_child("height").limit_to_first(2).get()
        )
        assert list(value.keys()) == self.height_sorted[:2]

    def test_limit_first_all(self, testref):
        value = (
            testref.child("dinosaurs").order_by_child("height").limit_to_first(10).get()
        )
        assert list(value.keys()) == self.height_sorted

    def test_limit_last(self, testref):
        value = (
            testref.child("dinosaurs").order_by_child("height").limit_to_last(2).get()
        )
        assert list(value.keys()) == self.height_sorted[-2:]

    def test_limit_last_all(self, testref):
        value = (
            testref.child("dinosaurs").order_by_child("height").limit_to_last(10).get()
        )
        assert list(value.keys()) == self.height_sorted
    ...
    def test_start_and_end_at(self, testref):
        value = (
            testref.child("dinosaurs")
            .order_by_child("height")
            .start_at(2.5)
            .end_at(5)
            .get()
        )
    ...
    def test_order_by_nested_child(self, testref):
        value = (
            testref.child("dinosaurs").order_by_child("ratings/pos").start_at(4).get()
        )
        assert len(value) == 3
