...
import pytest
from googleapiclient import _helpers, http

import firebase_admin
from firebase_admin import _http_client, exceptions, messaging
...
class TestMessageStr:
    ...
    def test_data_message(self):
        assert str(messaging.Message(topic="topic", data={})) == '{"topic": "topic"}'
        assert (
            str(messaging.Message(topic="topic", data={"k1": "v1", "k2": "v2"}))
            == '{"data": {"k1": "v1", "k2": "v2"}, "topic": "topic"}'
        )
...
class TestMessageEncoder:
    ...
    def test_fcm_options(self):
        check_encoding(
            ...
            {
                "topic": "topic",
                "fcm_options": {"analytics_label": "analytics_label_v1"},
            },
        )
        check_encoding(
...
class TestNotificationEncoder:
    ...
    def test_invalid_notification(self, data):
        with pytest.raises(ValueError) as excinfo:
            check_encoding(messaging.Message(topic="topic", notification=data))
    ...
    def test_notification_message(self):
        ...
        check_encoding(
            messaging.Message(
                topic="topic", notification=messaging.Notification("t", "b")
            ),
...
class TestFcmOptionEncoder:
    ...
    def test_invalid_fcm_options(self, label):
        with pytest.raises(ValueError) as excinfo:
            check_encoding(
                messaging.Message(
                    topic="topic", fcm_options=messaging.FCMOptions(label)
                )
            )
...
class TestAndroidConfigEncoder:
    ...
    def test_invalid_android(self, data):
        with pytest.raises(ValueError) as excinfo:
            check_encoding(messaging.Message(topic="topic", android=data))
    ...
    def test_invalid_priority(self, data):
        ...
        if isinstance(data, str):
            ...
            assert (
                str(excinfo.value)
                == "AndroidConfig.priority must be a non-empty string."
            )
    ...
    def test_invalid_ttl(self, data):
        ...
        if isinstance(data, numbers.Number):
            ...
            assert str(excinfo.value) == (
                "AndroidConfig.ttl must be a duration in seconds or "
    ...
    def test_invalid_package_name(self, data):
        with pytest.raises(ValueError) as excinfo:
            check_encoding(
                messaging.Message(
                    topic="topic",
                    android=messaging.AndroidConfig(restricted_package_name=data),
                )
            )
        assert (
            str(excinfo.value)
    ...
    def test_android_ttl(self, ttl):
        msg = messaging.Message(
            topic="topic", android=messaging.AndroidConfig(ttl=ttl[0])
        )
        expected = {
            "topic": "topic",
            "android": {
...
class TestAndroidNotificationEncoder:
    def _check_notification(self, notification):
        with pytest.raises(ValueError) as excinfo:
            check_encoding(
                messaging.Message(
                    topic="topic",
                    android=messaging.AndroidConfig(notification=notification),
                )
            )
        return excinfo
    ...
    @pytest.mark.parametrize(
        "data", NON_STRING_ARGS + ["foo", "#xxyyzz", "112233", "#11223"]
    )
    def test_invalid_color(self, data):
        ...
        if isinstance(data, str):
            ...
            assert (
                str(excinfo.value)
                == "AndroidNotification.color must be a non-empty string."
            )
    ...
    def test_invalid_click_action(self, data):
        ...
        excinfo = self._check_notification(notification)
        assert (
            str(excinfo.value) == "AndroidNotification.click_action must be a string."
        )
    ...
    def test_invalid_title_loc_key(self, data):
        ...
        excinfo = self._check_notification(notification)
        assert (
            str(excinfo.value) == "AndroidNotification.title_loc_key must be a string."
        )

    @pytest.mark.parametrize("data", NON_LIST_ARGS)
    def test_invalid_title_loc_args(self, data):
        notification = messaging.AndroidNotification(
            title_loc_key="foo", title_loc_args=data
        )
        excinfo = self._check_notification(notification)
        if isinstance(data, list):
            expected = (
                "AndroidNotification.title_loc_args must not contain non-string values."
            )
            assert str(excinfo.value) == expected
        else:
    ...
    def test_invalid_body_loc_key(self, data):
        ...
        excinfo = self._check_notification(notification)
        assert (
            str(excinfo.value) == "AndroidNotification.body_loc_key must be a string."
        )

    @pytest.mark.parametrize("data", NON_LIST_ARGS)
    def test_invalid_body_loc_args(self, data):
        notification = messaging.AndroidNotification(
            body_loc_key="foo", body_loc_args=data
        )
        excinfo = self._check_notification(notification)
        if isinstance(data, list):
            expected = (
                "AndroidNotification.body_loc_args must not contain non-string values."
            )
            assert str(excinfo.value) == expected
        else:
    ...
    def test_invalid_priority(self, priority):
        ...
        if isinstance(priority, str):
            if not priority:
                ...
                expected = (
                    'AndroidNotification.priority must be "default", "min", "low", "high" '
                    'or "max".'
                )
        else:
            expected = "AndroidNotification.priority must be a non-empty string."
    ...
    def test_invalid_visibility(self, visibility):
        ...
        if isinstance(visibility, str):
            if not visibility:
                ...
                expected = (
                    'AndroidNotification.visibility must be "private", "public" or'
                    ' "secret".'
                )
        else:
            expected = "AndroidNotification.visibility must be a non-empty string."
    ...
    @pytest.mark.parametrize(
        "vibrate_timings", ["", 1, True, "msec", ["500", 500], [0, "abc"]]
    )
    def test_invalid_vibrate_timings_millis(self, vibrate_timings):
        notification = messaging.AndroidNotification(
            vibrate_timings_millis=vibrate_timings
        )
        excinfo = self._check_notification(notification)
        if isinstance(vibrate_timings, list):
            expected = (
                "AndroidNotification.vibrate_timings_millis must not contain non-number "
                "values."
            )
        else:
            expected = (
                "AndroidNotification.vibrate_timings_millis must be a list of numbers."
            )
        assert str(excinfo.value) == expected
    ...
    def test_invalid_notification_count(self, notification_count):
        ...
        assert (
            str(excinfo.value)
            == "AndroidNotification.notification_count must be a number."
        )
...
class TestLightSettingsEncoder:
        ...
        with pytest.raises(ValueError) as excinfo:
            check_encoding(
                messaging.Message(
                    topic="topic",
                    android=messaging.AndroidConfig(
                        notification=messaging.AndroidNotification(light_settings=data)
                    ),
                )
            )
    ...
    def test_no_color(self):
        light_settings = messaging.LightSettings(
            color=None, light_on_duration_millis=200, light_off_duration_millis=200
        )
    ...
    def test_invalid_light_off_duration_millis(self, data):
        ...
        if isinstance(data, numbers.Number):
            assert str(excinfo.value) == (
                "LightSettings.light_off_duration_millis must not be " "negative."
            )
        else:
            assert str(excinfo.value) == (
                "LightSettings.light_off_duration_millis must be a "
    ...
    def test_invalid_light_on_duration_millis(self, data):
        ...
        if isinstance(data, numbers.Number):
            assert str(excinfo.value) == (
                "LightSettings.light_on_duration_millis must not be " "negative."
            )
        else:
            assert str(excinfo.value) == (
                "LightSettings.light_on_duration_millis must be a "
                "duration in milliseconds or "
                "an instance of datetime.timedelta."
            )
    ...
    def test_invalid_color(self, data):
        notification = messaging.LightSettings(
            color=data, light_on_duration_millis=300, light_off_duration_millis=200
        )
        ...
        if isinstance(data, str):
            ...
            assert (
                str(excinfo.value) == "LightSettings.color must be a non-empty string."
            )
...
class TestWebpushConfigEncoder:
    ...
    def test_invalid_webpush(self, data):
        with pytest.raises(ValueError) as excinfo:
            check_encoding(messaging.Message(topic="topic", webpush=data))
...
class TestWebpushNotificationEncoder:
    def _check_notification(self, notification):
        with pytest.raises(ValueError) as excinfo:
            check_encoding(
                messaging.Message(
                    topic="topic",
                    webpush=messaging.WebpushConfig(notification=notification),
                )
            )
        return excinfo
    ...
    def test_invalid_direction(self, data):
        ...
        if isinstance(data, str):
            assert str(excinfo.value) == (
                'WebpushNotification.direction must be "auto", ' '"ltr" or "rtl".'
            )
        else:
            assert (
                str(excinfo.value) == "WebpushNotification.direction must be a string."
            )
    ...
    def test_invalid_timestamp(self, data):
        ...
        assert (
            str(excinfo.value)
            == "WebpushNotification.timestamp_millis must be a number."
        )
    ...
    def test_invalid_actions(self, data):
        ...
        excinfo = self._check_notification(notification)
        assert str(excinfo.value) == (
            "WebpushConfig.notification.actions must be a list of "
    ...
    def test_invalid_action_name(self, data):
        ...
        excinfo = self._check_notification(notification)
        assert (
            str(excinfo.value) == "WebpushNotificationAction.action must be a string."
        )
    ...
    def test_invalid_action_icon(self, data):
        action = messaging.WebpushNotificationAction(
            action="action", title="title", icon=data
        )
        notification = messaging.WebpushNotification(actions=[action])
        excinfo = self._check_notification(notification)
...
class TestApsSoundEncoder:
                ...
                payload=messaging.APNSPayload(
                    aps=messaging.Aps(
                        sound=messaging.CriticalSound(
                            name="default", critical=True, volume=0.5
                        )
                    ),
                )
            ),
    ...
    def test_critical_sound_name_only(self):
        msg = messaging.Message(
            ...
            apns=messaging.APNSConfig(
                payload=messaging.APNSPayload(
                    aps=messaging.Aps(sound=messaging.CriticalSound(name="default")),
                )
            ),
        )
        expected = {
...
class TestSend:
    ...
    def _instrument_messaging_service(
        self, app=None, status=200, payload=_DEFAULT_RESPONSE
    ):
        if not app:
            app = firebase_admin.get_app()
        fcm_service = messaging._get_messaging_service(app)
    ...
    def test_invalid_send(self, msg):
        ...
        assert (
            str(excinfo.value)
            == "Message must be an instance of messaging.Message class."
        )
    ...
    def test_send_error(self, status, exc_type):
        ...
        with pytest.raises(exc_type) as excinfo:
            messaging.send(msg)
        expected = "Unexpected HTTP response with status: {0}; body: {{}}".format(
            status
        )
...
class TestBatch:
    ...
    def _instrument_batch_messaging_service(
        self, app=None, status=200, payload="", exc=None
    ):
...
Content-Type: application/json; charset=UTF-8\r\n\r\n{}\r\n\r\n"""
        for index, (status_code, content) in enumerate(payloads):
            payload += _playload_format.format(
                str(index + 1), str(status_code), content
            )
        payload += "--boundary--"
        return payload
...
class TestSendEach(TestBatch):
    ...
    def test_send_each_detailed_error(self, status):
        ...
        _ = self._instrument_messaging_service(
            response_dict={
                "foo1": [200, success_payload],
                "foo2": [status, error_payload],
            }
        )
        msg1 = messaging.Message(topic="foo1")
        msg2 = messaging.Message(topic="foo2")
    ...
    def test_send_all_detailed_error(self, status):
        ...
        _ = self._instrument_batch_messaging_service(
            payload=self._batch_payload(
                [(200, success_payload), (status, error_payload)]
            )
        )
        msg = messaging.Message(topic="foo")
...
class TestTopicManagement:
    ...
    def test_subscribe_to_topic_non_json_error(self, status, exc_type):
        ...
        with pytest.raises(exc_type) as excinfo:
            messaging.subscribe_to_topic("foo", "test-topic")
        reason = "Unexpected HTTP response with status: {0}; body: not json".format(
            status
        )
