...
def _get_initialized_app(app):
  if app is None:
    ...
    if app is not initialized_app:
      raise ValueError('Illegal app argument. App instance not initialized'
                       ' via the firebase module.')
    return app
  else:
...
class _TokenGenerator(object):
  ...
  FIREBASE_AUDIENCE = ('https://identitytoolkit.googleapis.com/google.identity.'
  ...
  def create_custom_token(self, uid, developer_claims=None):
    ...
    if developer_claims is not None:
      ...
      disallowed_keys = set(developer_claims.keys()) & self._RESERVED_CLAIMS_
      if disallowed_keys:
        if len(disallowed_keys) > 1:
          error_message = ('Developer claims {0} are reserved and cannot be '
                           'specified.'.format(', '.join(disallowed_keys)))
        else:
          error_message = ('Developer claim {0} is reserved and cannot be '
                           'specified.'.format(', '.join(disallowed_keys)))
  ...
  def verify_id_token(self, id_token):
    ...
    if not project_id:
      raise ValueError('Must initialize app with a CertificateCredential or '
                       'set your Firebase project ID as the GCLOUD_PROJECT '
                       'environment variable to call verify_id_token().')
    ...
    if not header.get('kid'):
      if audience == self.FIREBASE_AUDIENCE:
        error_message = ('verify_id_token() expects an ID token, but was'
                         ' given a custom token.')
      elif header.get('alg') == 'HS256' and payload.get(
          'v') is 0 and 'uid' in payload.get('d', {}):
        error_message = ('verify_id_token() expects an ID token, but was'
                         ' given a legacy custom token.')
      else:
      ...
      error_message = (
          ...
          '"{0}" but got "{1}". {2} {3}'.format(
              project_id, audience, project_id_match_msg, verify_id_token_msg))
    elif issuer != expected_issuer:
      error_message = ('Firebase ID token has incorrect "iss" (issuer) claim.'
      ...
      error_message = ('Firebase ID token has an empty string "sub" (subject) '
                       'claim. ') + verify_id_token_msg
    elif len(subject) > 128:
      error_message = ('Firebase ID token has "sub" (subject) claim longer than'
                       ' 128 characters. ') + verify_id_token_msg
...
class Credential(object):
  ...
  def get_credential(self):
    """Returns the underlying credential instance used for authentication."""
    raise NotImplementedError
...
class CertificateCredential(Credential):
  ...
  def __init__(self, file_path):
    ...
    try:
      ...
      err_message = 'Failed to parse the private key string: {0}'.format(error)
