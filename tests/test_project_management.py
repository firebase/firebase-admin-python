...
from firebase_admin import _http_client, exceptions, project_management
from tests import testutils

OPERATION_IN_PROGRESS_RESPONSE = json.dumps(
    {"name": "operations/abcdefg", "done": False}
)
...
LIST_ANDROID_APPS_RESPONSE = json.dumps(
    {
        "apps": [
            {
                "name": "projects/test-project-id/androidApps/1:12345678:android:deadbeef",
...
LIST_IOS_APPS_RESPONSE = json.dumps(
    {
        "apps": [
            {
                "name": "projects/test-project-id/iosApps/1:12345678:ios:ca5cade5",
...
TEST_APP_ENCODED_CONFIG = base64.standard_b64encode(
...
GET_SHA_CERTIFICATES_RESPONSE = json.dumps(
    {
        "certificates": [
...
class TestAndroidAppMetadata:
    ...
    def test_android_app_metadata_eq_and_hash(self):
        ...
        assert set([metadata_1, metadata_2, metadata_7]) == set(
...
class TestIOSAppMetadata:
    ...
    def test_ios_app_metadata_name(self):
        assert (
            IOS_APP_METADATA._name
            == "projects/test-project-id/iosApps/1:12345678:ios:ca5cade5"
        )
...
class TestSHACertificate:
    def test_create_sha_certificate_errors(self):
        ...
        with pytest.raises(ValueError):
            project_management.SHACertificate(
                sha_hash=0x123456789A123456789A123456789A123456789A
            )
        ...
        with pytest.raises(ValueError):
            project_management.SHACertificate(
                sha_hash="123456789a123456789a123456789a123456oops"
            )
    ...
    def test_sha_certificate_eq(self):
        ...
        sha_cert_4 = project_management.SHACertificate(
            "123456789a123456789a123456789a123456789a",
            "projects/-/androidApps/{0}/sha/notname1",
        )
    ...
    def test_sha_certificate_name(self):
        assert (
            SHA_1_CERTIFICATE.name
            == "projects/-/androidApps/1:12345678:android:deadbeef/sha/name1"
        )
        assert (
            SHA_256_CERTIFICATE.name
            == "projects/-/androidApps/1:12345678:android:deadbeef/sha/name256"
        )

    def test_sha_certificate_sha_hash(self):
        assert SHA_1_CERTIFICATE.sha_hash == "123456789a123456789a123456789a123456789a"
        assert (
            SHA_256_CERTIFICATE.sha_hash
            == "123456789a123456789a123456789a123456789a123456789a123456789a1234"
        )
...
class BaseProjectManagementTest:
    ...
    def _instrument_service(self, statuses, responses, app=None):
        ...
        project_management_service = project_management._get_project_management_service(
            app
        )
        recorder = []
...
class TestCreateAndroidApp(BaseProjectManagementTest):
    _CREATION_URL = (
        "https://firebase.googleapis.com/v1beta1/projects/test-project-id/androidApps"
    )
    ...
    def test_create_android_app(self):
        ...
        android_app = project_management.create_android_app(
            package_name="com.hello.world.android", display_name="My Android App"
        )
    ...
    def test_create_android_app_already_exists(self):
        recorder = self._instrument_service(
            statuses=[409], responses=[ALREADY_EXISTS_RESPONSE]
        )
    ...
    def test_create_android_app_polling_rpc_error(self):
        recorder = self._instrument_service(
            statuses=[
                ...
                200,
                503,
            ],  # Error 503 means that backend servers are over capacity.
            responses=[
                OPERATION_IN_PROGRESS_RESPONSE,  # Request to create Android app asynchronously.
                OPERATION_IN_PROGRESS_RESPONSE,  # Creation operation is still not done.
    ...
    def test_create_android_app_polling_failure(self):
        ...
        assert "Polling finished, but the operation terminated in an error" in str(
            excinfo.value
        )
...
class TestCreateIOSApp(BaseProjectManagementTest):
    _CREATION_URL = (
        "https://firebase.googleapis.com/v1beta1/projects/test-project-id/iosApps"
    )
    ...
    def test_create_ios_app_without_display_name(self):
        ...
        ios_app = project_management.create_ios_app(bundle_id="com.hello.world.ios")

        assert ios_app.app_id == "1:12345678:ios:ca5cade5"
        assert len(recorder) == 3
        body = {"bundleId": "com.hello.world.ios"}
        self._assert_request_is_correct(
            recorder[0], "POST", TestCreateIOSApp._CREATION_URL, body
        )
        self._assert_request_is_correct(
            recorder[1], "GET", "https://firebase.googleapis.com/v1/operations/abcdefg"
        )
    ...
    def test_create_ios_app(self):
        ...
        ios_app = project_management.create_ios_app(
            bundle_id="com.hello.world.ios", display_name="My iOS App"
        )
...
class TestListAndroidApps(BaseProjectManagementTest):
    _LISTING_URL = (
        "https://firebase.googleapis.com/v1beta1/projects/test-project-id/"
    ...
    def test_list_android_apps(self):
        ...
        expected_app_ids = set(
            ["1:12345678:android:deadbeef", "1:12345678:android:deadbeefcafe"]
        )
        assert set(app.app_id for app in android_apps) == expected_app_ids
        assert len(recorder) == 1
        self._assert_request_is_correct(
            recorder[0], "GET", TestListAndroidApps._LISTING_URL
        )

    def test_list_android_apps_rpc_error(self):
        recorder = self._instrument_service(
            statuses=[503], responses=[UNAVAILABLE_RESPONSE]
        )
    ...
    def test_list_android_apps_empty_list(self):
        recorder = self._instrument_service(
            statuses=[200], responses=[json.dumps(dict())]
        )
    ...
    def test_list_android_apps_multiple_pages(self):
        recorder = self._instrument_service(
            statuses=[200, 200],
            responses=[
                LIST_ANDROID_APPS_PAGE_1_RESPONSE,
                LIST_ANDROID_APPS_PAGE_2_RESPONSE,
            ],
        )
        ...
        self._assert_request_is_correct(
            recorder[1], "GET", TestListAndroidApps._LISTING_PAGE_2_URL
        )
...
class TestListIOSApps(BaseProjectManagementTest):
    ...
    def test_list_ios_apps(self):
        ...
        expected_app_ids = set(
            ["1:12345678:ios:ca5cade5", "1:12345678:ios:ca5cade5cafe"]
        )
        assert set(app.app_id for app in ios_apps) == expected_app_ids
        assert len(recorder) == 1
        self._assert_request_is_correct(
            recorder[0], "GET", TestListIOSApps._LISTING_URL
        )
    ...
    def test_list_ios_apps_multiple_pages(self):
        ...
        self._assert_request_is_correct(
            recorder[1], "GET", TestListIOSApps._LISTING_PAGE_2_URL
        )
...
class TestAndroidApp(BaseProjectManagementTest):
    ...
    def test_get_metadata_not_found(self, android_app):
        recorder = self._instrument_service(
            statuses=[404], responses=[NOT_FOUND_RESPONSE]
        )
    ...
    def test_get_config(self, android_app):
        ...
        assert len(recorder) == 1
        self._assert_request_is_correct(
            recorder[0], "GET", TestAndroidApp._GET_CONFIG_URL
        )
    ...
    def test_get_sha_certificates(self, android_app):
        ...
        assert len(recorder) == 1
        self._assert_request_is_correct(
            recorder[0], "GET", TestAndroidApp._LIST_CERTS_URL
        )
    ...
    def test_add_sha_1_certificate(self, android_app):
        ...
        assert len(recorder) == 1
        body = {
            "shaHash": "123456789a123456789a123456789a123456789a",
            "certType": "SHA_1",
        }
        self._assert_request_is_correct(
            recorder[0], "POST", TestAndroidApp._ADD_CERT_URL, body
        )
    ...
    def test_raises_if_app_has_no_project_id(self):
        def evaluate():
            ...
            with pytest.raises(ValueError):
                project_management.android_app(
                    app_id="1:12345678:android:deadbeef", app=app
                )
...
class TestIOSApp(BaseProjectManagementTest):
    ...
    def test_get_metadata_no_display_name(self, ios_app):
        ...
        assert (
            metadata._name == "projects/test-project-id/iosApps/1:12345678:ios:ca5cade5"
        )
        ...
        assert len(recorder) == 1
        self._assert_request_is_correct(
            recorder[0], "GET", TestIOSApp._GET_METADATA_URL
        )

    def test_get_metadata(self, ios_app):
        recorder = self._instrument_service(
            statuses=[200], responses=[IOS_APP_METADATA_RESPONSE]
        )
