...
def _instrument_user_manager(app, status, payload):
    ...
    user_manager.http_client.session.mount(
        TOKEN_MGT_URLS["ID_TOOLKIT"], testutils.MockAdapter(payload, status, recorder)
    )
    ...
    def test_sign_with_discovery_failure(self):
        ...
        try:
            ...
            assert str(excinfo.value).startswith(
                "Failed to determine service account: test error"
            )
...
class TestCreateSessionCookie:
    @pytest.mark.parametrize(
        "id_token", [None, "", 0, 1, True, False, list(), dict(), tuple()]
    )
    ...
    def test_invalid_expires_in(self, user_mgt_app, expires_in):
        with pytest.raises(ValueError):
            auth.create_session_cookie(
                "id_token", expires_in=expires_in, app=user_mgt_app
            )
    ...
    def test_valid_args(self, user_mgt_app, expires_in):
        _, recorder = _instrument_user_manager(
            user_mgt_app, 200, '{"sessionCookie": "cookie"}'
        )
    ...
    def test_error(self, user_mgt_app):
        ...
        assert (
            str(excinfo.value) == "The provided ID token is invalid (INVALID_ID_TOKEN)."
        )
    ...
    def test_error_with_details(self, user_mgt_app):
        _instrument_user_manager(
            user_mgt_app,
            500,
            '{"error":{"message": "INVALID_ID_TOKEN: More details."}}',
        )
        with pytest.raises(auth.InvalidIdTokenError) as excinfo:
            auth.create_session_cookie("id_token", expires_in=3600, app=user_mgt_app)
        assert excinfo.value.code == exceptions.INVALID_ARGUMENT
    ...
    def test_unexpected_error_code(self, user_mgt_app):
        _instrument_user_manager(
            user_mgt_app, 500, '{"error":{"message": "SOMETHING_UNUSUAL"}}'
        )
...
class TestVerifyIdToken:
    ...
    invalid_tokens = {
        ...
        "ExpiredToken": _get_id_token(
            {"iat": int(time.time()) - 10000, "exp": int(time.time()) - 3600}
        ),
        "ExpiredTokenShort": _get_id_token(
            {"iat": int(time.time()) - 10000, "exp": int(time.time()) - 30}
        ),
    ...
    @pytest.mark.parametrize("id_token", valid_tokens.values(), ids=list(valid_tokens))
    def test_revoked_token_do_not_check_revoked(
        self, user_mgt_app, revoked_tokens, id_token
    ):
        _overwrite_cert_request(user_mgt_app, MOCK_REQUEST)
        _instrument_user_manager(user_mgt_app, 200, revoked_tokens)
    ...
    def test_disabled_user_do_not_check_revoked(
        self, user_mgt_app, user_disabled, id_token
    ):
        _overwrite_cert_request(user_mgt_app, MOCK_REQUEST)
        _instrument_user_manager(user_mgt_app, 200, user_disabled)
        claims = auth.verify_id_token(id_token, app=user_mgt_app, check_revoked=False)
    ...
    def test_expired_token_with_tolerance(self, user_mgt_app):
        ...
        if _is_emulated():
            self._assert_valid_token(id_token, user_mgt_app)
            return
        claims = auth.verify_id_token(id_token, app=user_mgt_app, clock_skew_seconds=60)
        assert claims["admin"] is True
        assert claims["uid"] == claims["sub"]
        with pytest.raises(auth.ExpiredIdTokenError):
            auth.verify_id_token(id_token, app=user_mgt_app, clock_skew_seconds=20)
...
class TestVerifySessionCookie:
    ...
    def _assert_valid_cookie(self, cookie, app, check_revoked=False):
        claims = auth.verify_session_cookie(
            cookie, app=app, check_revoked=check_revoked
        )
        assert claims["admin"] is True
        assert claims["uid"] == claims["sub"]
    ...
    @pytest.mark.parametrize("cookie", valid_cookies.values(), ids=list(valid_cookies))
    def test_revoked_cookie_does_not_check_revoked(
        self, user_mgt_app, revoked_tokens, cookie
    ):
        _overwrite_cert_request(user_mgt_app, MOCK_REQUEST)
        _instrument_user_manager(user_mgt_app, 200, revoked_tokens)
    ...
    def test_disabled_user_does_not_check_revoked(
        self, user_mgt_app, user_disabled, cookie
    ):
        _overwrite_cert_request(user_mgt_app, MOCK_REQUEST)
        _instrument_user_manager(user_mgt_app, 200, user_disabled)
        self._assert_valid_cookie(cookie, app=user_mgt_app, check_revoked=False)
    ...
    def test_expired_cookie_with_tolerance(self, user_mgt_app):
        ...
        if _is_emulated():
            ...
            return
        claims = auth.verify_session_cookie(
            cookie, app=user_mgt_app, check_revoked=False, clock_skew_seconds=59
        )
        assert claims["admin"] is True
        assert claims["uid"] == claims["sub"]
        with pytest.raises(auth.ExpiredSessionCookieError):
            auth.verify_session_cookie(
                cookie, app=user_mgt_app, check_revoked=False, clock_skew_seconds=29
            )
    ...
    @pytest.mark.parametrize(
        "env_var_app", [{"GCLOUD_PROJECT": "mock-project-id"}], indirect=True
    )
    def test_project_id_env_var(self, env_var_app):
        _overwrite_cert_request(env_var_app, MOCK_REQUEST)
...
class TestCertificateCaching:
    def test_certificate_caching(self, user_mgt_app, httpserver):
        httpserver.serve_content(
            MOCK_PUBLIC_CERTS, 200, headers={"Cache-Control": "max-age=3600"}
        )
...
class TestCertificateFetchTimeout:
    ...
    def _instrument_session(self, app):
        ...
        request.session.mount(
            "https://", testutils.MockAdapter(MOCK_PUBLIC_CERTS, 200, recorder)
        )
        return recorder
