...
from tests import test_token_gen, testutils
...
USER_MGT_URL_PREFIX = (
    "https://identitytoolkit.googleapis.com/v1/projects/mock-project-id"
)
PROVIDER_MGT_URL_PREFIX = (
    "https://identitytoolkit.googleapis.com/v2/projects/mock-project-id"
)
TENANT_MGT_URL_PREFIX = (
    "https://identitytoolkit.googleapis.com/v2/projects/mock-project-id"
)
...
class TestTenant:
    @pytest.mark.parametrize(
        "data", [None, "foo", 0, 1, True, False, list(), tuple(), dict()]
    )
...
class TestTenantAwareUserManagement:
    ...
    def test_get_user_by_phone_number(self, tenant_mgt_app):
        ...
        assert user.email == "testuser@example.com"
        self._assert_request(
            recorder, "/accounts:lookup", {"phoneNumber": ["+1234567890"]}
        )
    ...
    def test_list_users(self, tenant_mgt_app):
        ...
        assert (
            req.url
            == "{0}/tenants/tenant-id/accounts:batchGet?maxResults=1000".format(
                USER_MGT_URL_PREFIX
            )
        )
    ...
    def test_get_saml_provider_config(self, tenant_mgt_app):
        ...
        assert (
            req.url
            == "{0}/tenants/tenant-id/inboundSamlConfigs/saml.provider".format(
                PROVIDER_MGT_URL_PREFIX
            )
        )
    ...
    def test_create_saml_provider_config(self, tenant_mgt_app):
        ...
        self._assert_request(
            recorder,
            "/inboundSamlConfigs?inboundSamlConfigId=saml.provider",
    ...
    def test_list_saml_provider_configs(self, tenant_mgt_app):
        ...
        assert req.url == "{0}{1}".format(
            PROVIDER_MGT_URL_PREFIX,
            "/tenants/tenant-id/inboundSamlConfigs?pageSize=100",
        )
