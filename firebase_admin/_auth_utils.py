...
def validate_phone(phone, required=False):
    ...
    if not isinstance(phone, str) or not phone:
        raise ValueError(
            'Invalid phone number: "{0}". Phone number must be a non-empty '
            "string.".format(phone)
        )
    if not phone.startswith("+") or not re.search("[a-zA-Z0-9]", phone):
        raise ValueError(
            'Invalid phone number: "{0}". Phone number must be a valid, E.164 '
...
def validate_photo_url(photo_url, required=False):
    ...
    if not isinstance(photo_url, str) or not photo_url:
        raise ValueError(
            'Invalid photo URL: "{0}". Photo URL must be a non-empty ' "string.".format(
                photo_url
            )
        )
    try:
...
def validate_action_type(action_type):
    if action_type not in VALID_EMAIL_ACTION_TYPES:
        raise ValueError(
            "Invalid action type provided action_type: {0}. \
            Valid values are {1}".format(
                action_type, ", ".join(VALID_EMAIL_ACTION_TYPES)
            )
        )
    return action_type
...
def _build_error_message(code, exc_type, custom_message):
    default_message = (
        exc_type.default_message
        if (exc_type and hasattr(exc_type, "default_message"))
        else "Error while calling Auth service"
    )
