...
def test_create_oidc_provider_config(oidc_provider):
    ...
    assert oidc_provider.response_type.id_token == False
...
def test_update_oidc_provider_config():
    ...
    try:
        ...
        assert provider_config.client_secret == True
        assert oidc_provider.response_type.id_token == False
        assert oidc_provider.response_type.code == True
        assert oidc_provider.client_secret == 'CLIENT_SECRET'
    finally:
        auth.delete_oidc_provider_config(provider_config.provider_id)
