...
class _AppCheckService:
    ...
    def verify_token(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Verifying App Check token failed. Error: {exception}")
    ...
    def _has_valid_token_headers(self, headers: Any) -> None:
        ...
        if headers.get("typ") != "JWT":
            raise ValueError(
                "The provided App Check token has an incorrect type header"
            )
    ...
    def _decode_and_verify(self, token: str, signing_key: str):
        ...
        try:
            ...
            raise ValueError("The provided App Check token has an invalid signature.")
            ...
            raise ValueError("The provided App Check token has expired.")
        except InvalidTokenError as exception:
            raise ValueError(f"Decoding App Check token failed. Error: {exception}")

        audience = payload.get("aud")
        if not isinstance(audience, list) or self._scoped_project_id not in audience:
            raise ValueError(
                'Firebase App Check token has incorrect "aud" (audience) claim.'
            )
        ...
        _Validators.check_string(
            'The provided App Check token "sub" (subject) claim', payload.get("sub")
        )
...
class _Validators:
    ...
    def check_string(cls, label: str, value: Any):
        ...
        if value is None:
            raise ValueError(
                '{0} "{1}" must be a non-empty string.'.format(label, value)
            )
