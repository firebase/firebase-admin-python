...
from firebase_admin import _http_client, _sseclient, _utils, db, exceptions
...
class TestReference:
    ...
    def setup_class(cls):
        firebase_admin.initialize_app(
            testutils.MockCredential(), {"databaseURL": cls.test_url}
        )
    ...
    @pytest.mark.parametrize("value", [_Object(), {"foo": _Object()}, [_Object()]])
    ...
    @pytest.mark.parametrize(
        "update", [None, {}, {None: "foo"}, "", "foo", 0, 1, list(), tuple(), _Object()]
    )
    ...
    @pytest.mark.parametrize(
        "func", [None, 0, 1, True, False, "foo", dict(), list(), tuple()]
    )
    ...
    def test_other_error(self, error_code, func):
        ...
        assert (
            str(excinfo.value)
            == "Unexpected response from database: custom error message"
        )
        assert excinfo.value.cause is not None
        assert excinfo.value.http_response is not None
...
class TestReferenceWithAuthOverride:
    ...
    def test_set_value(self):
        ...
        data = {"foo": "bar"}
        ref.set(data)
        query_str = "print=silent&auth_variable_override={0}".format(
    ...
    def test_order_by_query(self):
        ...
        recorder = self.instrument(ref, json.dumps("data"))
        query = ref.order_by_child("foo")
        query_str = "orderBy=%22foo%22&auth_variable_override={0}".format(
            self.encoded_override
        )
        assert query.get() == "data"
        assert len(recorder) == 1
    ...
    def test_range_query(self):
        ...
        query_str = (
            "endAt=10&orderBy=%22foo%22&startAt=1&"
...
class TestDatabaseInitialization:
    ...
    def test_parse_db_url(
        self, url, emulator_host, expected_base_url, expected_namespace
    ):
        if emulator_host:
        ...
        try:
            firebase_admin.initialize_app(
                testutils.MockCredential(), {"databaseURL": url}
            )
            ref = db.reference()
            ...
            if expected_base_url.startswith("http://localhost"):
                assert isinstance(
                    ref._client.credential, _utils.EmulatorAdminCredentials
                )
            else:
                assert isinstance(
                    ref._client.credential, testutils.MockGoogleCredential
                )
        finally:
    ...
    @pytest.mark.parametrize(
        ...
        [
            "https://test.firebaseio.com",
            "https://test.firebaseio.com/",
            "https://test.eu-west1.firebasdatabase.app",
            "https://test.eu-west1.firebasdatabase.app/",
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "",
            "foo",
            "http://test.firebaseio.com",
            "http://test.firebasedatabase.app",
            True,
            False,
            1,
            0,
    ...
    def test_invalid_db_url(self, url):
        ...
        other_app = firebase_admin.initialize_app(
            testutils.MockCredential(), name="otherApp"
        )
        with pytest.raises(ValueError):
    ...
    @pytest.mark.parametrize(
        "override", ["", "foo", 0, 1, True, False, list(), tuple(), _Object()]
    )
...
class TestQuery:
    ...
    valid_paths = {"foo": "foo", "foo/bar": "foo/bar", "foo/bar/": "foo/bar"}
    ...
    @pytest.mark.parametrize(
        "limit", [None, -1, "foo", 1.2, list(), dict(), tuple(), _Object()]
    )
    ...
    def test_range_query(self, initquery):
        ...
        query.equal_to(2)
        query.end_at(3)
        assert query._querystr == 'endAt=3&equalTo=2&orderBy="{0}"&startAt=1'.format(
            order_by
        )
    ...
    def test_all_in(self, initquery):
        ...
        expected = 'endAt=3&equalTo=2&limitToFirst=10&orderBy="{0}"&startAt=1'.format(
            order_by
        )
