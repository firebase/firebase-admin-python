...
def input_stages():
    ...
    results = client.pipeline().documents(
...
def unnest_stage_empty_or_non_array():
    ...
    results = client.pipeline() \
        .database() \
        .unnest(Field.of("neighbors").as_("unnestedNeighbors"), \
            options=UnnestOptions(index_field="index")) \
        .execute()
...
def count_function():
    ...
    count_all = client.pipeline() \
        .collection("books") \
        .aggregate(Count().as_("count")) \
        .execute()

    # Number of books with nonnull `ratings` field
    count_field = client.pipeline() \
        .collection("books") \
...
def pow_function():
    ...
    result = client.pipeline() \
        .collection("cities") \
        .add_fields(
            Field.of("lat").subtract(googleplexLat)
                .multiply(111)  # km per degree
                .pow(2)
                .as_("latitudeDifference"),
            Field.of("lng").subtract(googleplexLng)
                .multiply(111)  # km per degree
                .pow(2)
