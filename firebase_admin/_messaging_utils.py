...
class Message(object):
    ...
    def __str__(self):
        import firebase_admin._messaging_encoder as _messaging_encoder
        return json.dumps(self, cls=_messaging_encoder.MessageEncoder, sort_keys=True)
...
class MulticastMessage(object):
    ...
    def __init__(self, tokens, data=None, notification=None, android=None, webpush=None, apns=None,
                 fcm_options=None):
        import firebase_admin._messaging_encoder as _messaging_encoder
        _messaging_encoder.Validators.check_string_list('MulticastMessage.tokens', tokens)
        if len(tokens) > 500:
