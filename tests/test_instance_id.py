...
from firebase_admin import _http_client, exceptions, instance_id
...
http_errors = {
    ...
    409: ('Instance ID "test_iid": Already deleted.', exceptions.ConflictError),
    429: (
        'Instance ID "test_iid": Request throttled out by the backend server.',
        exceptions.ResourceExhaustedError,
    ),
    500: ('Instance ID "test_iid": Internal server error.', exceptions.InternalError),
    503: (
...
class TestDeleteInstanceId:
    ...
    def _get_url(self, project_id, iid):
        return instance_id._IID_SERVICE_URL + "project/{0}/instanceId/{1}".format(
            project_id, iid
        )
    ...
    def test_delete_instance_id_unexpected_error(self):
        ...
        message = 'Instance ID "test_iid": 501 Server Error: None for url: {0}'.format(
            url
        )
    ...
    @pytest.mark.parametrize(
        "iid", [None, "", 0, 1, True, False, list(), dict(), tuple()]
    )
