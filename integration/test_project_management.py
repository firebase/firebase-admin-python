...
from firebase_admin import exceptions, project_management
...
def android_app(default_app):
    ...
    for android_app in android_apps:
        if _starts_with(
            android_app.get_metadata().display_name, TEST_APP_DISPLAY_NAME_PREFIX
        ):
            return android_app
...
def ios_app(default_app):
    ...
    for ios_app in ios_apps:
        if _starts_with(
            ios_app.get_metadata().display_name, TEST_APP_DISPLAY_NAME_PREFIX
        ):
            return ios_app
...
def test_android_sha_certificates(android_app):
    ...
    sha_256_hashes = set(
        cert.sha_hash for cert in cert_list if cert.cert_type == SHA_256
    )
...
def test_list_ios_apps(ios_app):
    ...
    assert any(
        _starts_with(ios_app.get_metadata().display_name, TEST_APP_DISPLAY_NAME_PREFIX)
        for ios_app in ios_apps
    )
