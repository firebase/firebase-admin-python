...
def send_each_for_multicast(multicast_message, dry_run=False, app=None):
    ...
    if not isinstance(multicast_message, MulticastMessage):
        raise ValueError(
            "Message must be an instance of messaging.MulticastMessage class."
        )
    messages = [
        Message(
...
def send_all(messages, dry_run=False, app=None):
    ...
    """
    warnings.warn(
        "send_all() is deprecated. Use send_each() instead.", DeprecationWarning
    )
...
class _MessagingService:
    ...
    def send(self, message, dry_run=False):
        ...
        try:
            resp = self._client.body(
                "post", url=self._fcm_url, headers=self._fcm_headers, json=data
            )
    ...
    def send_each(self, messages, dry_run=False):
        ...
        def send_data(data):
            try:
                ...
                return SendResponse(
                    resp=None, exception=self._handle_fcm_error(exception)
                )
            else:
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=len(message_data)
            ) as executor:
                responses = [resp for resp in executor.map(send_data, message_data)]
                return BatchResponse(responses)
        except Exception as error:
            raise exceptions.UnknownError(
                message="Unknown error while making remote service calls: {0}".format(
                    error
                ),
                cause=error,
            )
    ...
    def _build_fcm_error_requests(cls, error, message, error_dict):
        ...
        return (
            exc_type(message, cause=error, http_response=error.response)
            if exc_type
            else None
        )
    ...
    def _build_fcm_error_googleapiclient(
        cls, error, message, error_dict, http_response
    ):
        """Parses an error response from the FCM API and creates a FCM-specific exception if
        appropriate."""
        exc_type = cls._build_fcm_error(error_dict)
        return (
            exc_type(message, cause=error, http_response=http_response)
            if exc_type
            else None
        )
