...
def initialize_app(options, name=_DEFAULT_APP_NAME):
  ...
  if name == _DEFAULT_APP_NAME:
    raise ValueError((
        ...
        'initialize_app() more than once without providing an app name as the '
        'second argument. In most cases you only need to call initialize_app()'
        ' once. But if you do want to initialize multiple apps, pass a second '
        'argument to initialize_app() to give each app a unique name.'))
  else:
    raise ValueError((
        'Firebase app named "{0}" already exists. This means you called '
        'initialize_app() more than once with the same app name as the second '
        'argument. Make sure you provide a unique name every time you call '
        'initialize_app().').format(name))
...
def delete_app(name):
  ...
  if name == _DEFAULT_APP_NAME:
    raise ValueError(
        ('The default Firebase app does not exist. Make sure to initialize the'
         ' SDK by calling initialize_app().'))
  else:
    raise ValueError(
        ('Firebase app named "{0}" does not exist. Make sure to initialize the'
         ' SDK by calling initialize_app() with your app name as the second'
...
class App(object):
  ...
  def __init__(self, name, options):
    ...
    if not name or not isinstance(name, basestring):
      raise ValueError('Illegal Firebase app name "{0}" provided. '
                       'App name must be a non-empty string.'.format(name))
    self._name = name
